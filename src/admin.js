...
function sizeRow(size = { label: "", dimensions: "", price: null }) {
  ...
  row.innerHTML = `<div class="field"><span>Label</span><input data-key="label" value="${escapeHtml(size.label)}"></div><div class="field"><span>Dimensions</span><input data-key="dimensions" value="${escapeHtml(size.dimensions)}"></div><div class="field"><span>Price</span><input data-key="price" type="number" min="0" step="1" value="${size.price ?? ""}" placeholder="Quote"></div><button class="button danger remove" type="button">Remove</button>`;
  row.querySelector(".remove").addEventListener("click", () => row.remove());
  return row;
}

function artCard(artwork = {}) {
  const art = { id: uid(), title: "", story: "", originalSize: "", image: "", palette: "linear-gradient(145deg,#173f4f,#61aab1,#ed6847)", accent: "#d6ff45", active: true, ...artwork };
  const card = document.createElement("article");
  card.className = "art-card";
  card.dataset.id = art.id;
  card.innerHTML = `<div><div class="art-preview">${art.image ? `<img src="${escapeHtml(art.image)}" alt="">` : "No image yet"}</div><div class="upload-row"><input class="image-file" type="file" accept="image/png,image/jpeg,image/webp,image/gif"><button class="button secondary upload" type="button">Upload</button></div></div><div class="art-fields"><div class="field"><span>Title</span><input data-key="title" value="${escapeHtml(art.title)}"></div><div class="field"><span>Original size</span><input data-key="originalSize" value="${escapeHtml(art.originalSize)}"></div><div class="field full"><span>Description</span><textarea data-key="story">${escapeHtml(art.story)}</textarea></div><div class="field"><span>Placeholder background</span><input data-key="palette" value="${escapeHtml(art.palette)}"></div><div class="field"><span>Accent colour</span><input data-key="accent" type="color" value="${escapeHtml(art.accent || "#d6ff45")}"></div><input data-key="image" type="hidden" value="${escapeHtml(art.image)}"><label class="toggle"><input data-key="active" type="checkbox" ${art.active !== false ? "checked" : ""}> Show in shop</label><button class="button danger remove-art" type="button">Remove artwork</button></div>`;
...
function collectStore() {
  return {
    ...
    printSizes: [...document.querySelectorAll(".size-row")].map((row) => ({
...
async function saveStore() {
  ...
  try {
    ...
    if (!next.printSizes.length || !next.artworks.length) throw new Error("Keep at least one print size and one artwork.");
